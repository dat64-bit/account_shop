          ...
          {cartItems.length === 0 ? (
            ...
            <>
              <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
                ...
                {cartItems.map((item) => (
                  <div key={`${item.productId}-${item.planId}`} style={{ display: 'flex', gap: '15px', marginBottom: '20px', paddingBottom: '20px', borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ padding: '20px', background: '#f8fafc', borderTop: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', fontSize: '18px', fontWeight: '600' }}>
                  <span>Tổng tiền:</span>
                  <span style={{ color: 'var(--primary)' }}>{cartTotal.toLocaleString('vi-VN')}đ</span>
                </div>
                <button 
                  className="btn-primary" 
                  style={{ width: '100%', padding: '14px', fontSize: '16px', opacity: checkoutStatus === 'processing' ? 0.7 : 1 }}
                  onClick={handleCheckout}
                  disabled={checkoutStatus === 'processing'}
                >
                  {checkoutStatus === 'processing' ? 'Đang xử lý...' : 'THANH TOÁN'}
                </button>
              </div>
            </>
          )}
